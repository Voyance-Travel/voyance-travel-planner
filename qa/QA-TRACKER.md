...
| Smart Finish | ⬜ | ⬜ | no distinct "Smart Finish" CTA in current build — Trip Completion panel flags "N issues" but no separate finish button found | | ⬜ not surfaced in UI |
| Mystery activity | ⬜ | ⬜ | not surfaced in current build (likely feature-flagged off) | | ⬜ not surfaced in UI |
| Route optimization | ⬜ | ⬜ | (cross-ref C-COST-5) — no "optimize order" CTA found; per-leg routing works (TransitModePicker, real modes/times/costs) | | ⬜ no reorder-CTA; routing ✅ |
