...
- **🔴 OPEN + ROOT-CAUSE PINNED — EditorialItinerary saves fail to send (reorder doesn't persist).** Drove a confirmed reorder: UI reorders (Luggage Drop→8:30, Breakfast→9:30), an **Undo** chip appears, but the DB has the ORIGINAL order/times in **both** the JSON and the `itinerary_activities` table (the whole reorder reverts, not just the meal). Ruled OUT: payload size (itinerary_data only **55 kB**), the frozen guard (`user-` saveReason whitelisted), the day-count-shrink guard (same 4 days), and meal-time normalization (both activities reverted, not just the meal). **The actual failure:** the reorder fires **NO `generate-itinerary` network request at all**, and the console shows **`[EditorialItinerary] Backend save failed: FunctionsFetchError: Failed to send a request to the Edge Function`** — i.e. `persistDaysImmediately → supabase.functions.invoke('generate-itinerary', {action:'save-itinerary'})` is **failing at the transport layer / not sending**. The same function works for generate-trip + unlock, so it's specific to this invoke. **⚠️ Likely broader than reorder:** the autosave + AI-note + lock-toggle paths use the same `save-itinerary` invoke, so **all in-itinerary edits may be failing to persist** (cost still changes because `syncBudgetFromDays` writes `activity_costs` via a *direct* REST call, not via save-itinerary). **Next:** live-debug why the save-itinerary invoke throws FunctionsFetchError (transient network/abort vs a request-construction bug) — add a console probe around the invoke, or check if a shared AbortController/signal cancels it.
