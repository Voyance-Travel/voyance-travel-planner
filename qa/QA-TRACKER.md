...
| 🍽️ Preference adherence (dietary) | 🛠️ **FIX SHIPPED 2026-06-07 (`f6678c3ef`)** — root cause: conversational extractor had no dietary field + profile-loader ignored per-trip dietary. Now captured + merged + hard-enforced via existing buildDietaryEnforcementPrompt. **Frontend live; ⏳ edge fns (chat-trip-planner, generate-itinerary) PENDING DEPLOY — needs owner Supabase token** (→ C-PREF-1). |
...
| 🔵 Product QA on new stack | **~52%** — ✅ green: core build/money path, **all 4 build modes** (TEST PLAN §4), **generation correctness original-5 re-verified** (D4, 7+ fresh gens), **build wizard steps** (D2), **DNA→output differentiation** (B3/§1/§3), **in-itinerary persistence (incl. swap C-TOOL-8 fixed) + cost + day-unlock + restaurant-recs** (D5), **nav spot-check** (Explore dropdown, /archetypes, profile Overview/Credits/Preferences — no dead ends). 🛠️ dietary hard-filter shipped (edge deploy pending). **STILL UNTESTED LIVE:** prefs pace/budget/accessibility→output, D5 tools add/smart-finish/route-opt/hotel-opt/AI-chat/notes/export/maps, deeper nav (Friends/Following/footer), DNA-accuracy edge cases (C-DNA-2/3), auth-flow tails. |

**Now working through (owner-directed 2026-06-07 PM):** (1) ✅ tracker headline reflects build-mode + DNA wins; (2) ✅ preferences→output (D3) tested; (3) ✅ per-tool charges tested; (4) ✅✅ **C-TOOL-8 swap-persist FIXED+VERIFIED** + ALL edit handlers hardened (`9239a1901`); (5) 🛠️ **C-PREF-1 dietary HARD-filter fix SHIPPED** (`f6678c3ef`) — ⏳ edge deploy pending owner token; (6) ✅ nav dead-ends spot-checked (no dead ends); (7) DNA-accuracy: live DNA card shows calibrated "71% match + secondary archetype"; quiz re-take skipped (would overwrite restored DNA). **⛔ BLOCKER: edge functions need `supabase functions deploy chat-trip-planner generate-itinerary` to make the dietary fix live.**
...
| Hotel optimization | ⬜ | ⬜ | | | ⬜ |
| AI chat / trip-planner (itinerary-chat) | ✅ | 🔴 | **Chat (text) works** — context-aware ("Day 1 has 11 activities"), free, review-first proposal renders correctly. **BUT "Apply" action FAILS** — spend-credits 400 + generate-itinerary 403 (→ C-CHAT-1). No wrongful charge. | needs edge logs / deploy token to fix (C-CHAT-1) | 🔴 **Apply broken (C-CHAT-1)** |
...
| C-CHAT-1 | **HIGH** | AI Trip Assistant | **"Apply" on a Trip-Assistant suggestion FAILS.** Live (Florence ccfc4491): asked the assistant to remove an activity → it correctly proposed "Rewrite Day 1 · 30 cr" (review-first flow works) → clicked **Apply** → **Failed**. Network: `spend-credits` **400** + `generate-itinerary` **403** + persist `FunctionsFetchError`. Correctly did NOT charge (balance held 4,880). Ruled out: frontend ACTION_MAP is correct (`REGENERATE_DAY`→`regenerate_day`), session valid (balance loads 200). Suspect: the `rewrite_day` execute/persist path is forbidden for a `status='partial'`/draft trip, and/or InlineModifier omits `dayIndex`/`creditsAmount` for regenerate_day. ✅ found live 2026-06-07. NOTE: chat (text) is free + works; only the **Apply action** fails. | needs edge logs to pin the 403/400 — **confirm + fix BLOCKED on Supabase deploy token** (same blocker as C-PREF-1). Re-test on a clean (non-degraded) trip once deployable. | ⬜ |
