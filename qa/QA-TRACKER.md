...
| 🔵 Product QA on new stack | **~58%** — ✅ green: core build/money path, **all 4 build modes**, **generation correctness original-5 re-verified**, **build wizard steps** (D2), **DNA→output differentiation** (B3), **dietary HARD-filter** (C-PREF-1 ✅✅ deployed+verified: vegan Bologna = 100% vegan), **in-itinerary persistence (swap C-TOOL-8 fixed + ALL handlers hardened) + cost + day-unlock + restaurant-recs + add-activity + AI-Assistant Apply** (D5), **nav spot-check** (no dead ends). **STILL UNTESTED LIVE:** prefs pace/budget/accessibility→output, D5 tools smart-finish/route-opt/hotel-opt/notes/export/maps, deeper nav (Friends/Following/footer), DNA-accuracy edge cases (C-DNA-2/3), auth-flow tails. |
