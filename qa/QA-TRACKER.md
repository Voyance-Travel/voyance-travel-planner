...
- **C-CRED-4/8** (trip-gen server undercharge) — ✅ **IMPLEMENTED + DEPLOYED 2026-06-07** (commit `7b2b78653`); ⏳ **final live paid-charge test pending**. Math verified 10/10 vs the matrix; existing-trip parity confirmed (Madrid: 4d/1city/no-hotel/cost_dna-NULL → server computes **240**, == displayed). Server now recomputes + charges authoritative from the `cost_dna` snapshot; client display + authorize read the same snapshot (parity by construction). Backfill-safe (NULL→1.0×). **Remaining:** a live paid 2nd-trip with a multiplier-triggering config (special-occasion + ≥2 must-dos) to visually confirm charge==display==1.15× (Ashton's `first_trip_used` is still false → next gen is the free path, so needs a consumed-first-trip/2nd account). **Known follow-ups:** dietary factor needs a user_preferences fetch at creation; journey legs stay client-trusted (excluded from recompute); `budget_include_hotel` default=true (flagged).
