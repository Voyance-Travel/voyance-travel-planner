...
- **C-CRED-4/8** (trip-gen server undercharge) — ⏳ **IN PROGRESS** (owner chose "full snapshot"). Design = new `trips.cost_dna` jsonb column written at creation from the SAME DNA object the client displays, read by both client display and the server charge → **display == charge by construction**. Server recomputes authoritative cost from `cost_dna` + `trip_cities` count + `metadata.mustDoActivities` (via `extract-must-dos`) + `budget_include_hotel` + dates, and charges that (ignores client `creditsAmount`); partial/journey paths keep the legacy floor (proration preserved). **Closes the real exploit** (skipping the multi-city fee). **Findings:** `budget='strict'` is dead vocabulary (never fires); real multiplier drivers = special-occasion + ≥2 must-includes (+ dietary only if free-text restrictions are mapped). Files: migration (cost_dna) · `_shared/trip-cost.ts` (new) · vendored client must-do counter (parity) · `src/lib/costDna.ts` · spend-credits · Start.tsx + splitJourneyIfNeeded (snapshot) · ItineraryGenerator (display+authorize). **Money-critical → test against 12-row matrix before any charge.**
- **C-CRED-6** (monthly free-grant race → double 150cr): atomic conditional-UPDATE claim ready; prereq verified (all users have a credit_balances row). Queued.
- **C-CRED-2b** (guide dup-click double-charge): server-side idempotency claim row in generate-travel-guide. Queued.
