...
- **🔴 OPEN — reorder ORDER doesn't persist to the JSON.** The UI reorders correctly (Luggage Drop → 8:30, Breakfast → 9:30) but the DB `itinerary_data.days[0].activities` order is unchanged (still Breakfast, Check-in). `persistDaysImmediately(newDays)` (commit `b7ab6807e`) is called but the reordered order doesn't survive to the DB JSON — likely normalized away by `save-itinerary` and/or the read-time `itineraryParser` (which dedupes/strips/re-times on every load). Separate C-PERSIST-family bug; needs a focused look at `save-itinerary` activity-order handling.
