...
| C-COST-4 | MED | cost | frontend "Search with Google" button — untracked + exposed key + ceiling-bypass (keystrokes = free Nominatim, OK) | ✅ | ⬜ | server `places-search-proxy`; drop `VITE_GOOGLE_MAPS_API_KEY` path | ⬜ **OPEN — deferred this pass** (frontend work) |
| C-COST-5 | MED | cost | geocoding/routes/distance-matrix uncached (optimize/transit/transfers/airport) | ✅ | ⬜ | add `cachedGoogleGeocode/Routes/DistanceMatrix` wrappers — **note:** several callers (optimize-itinerary) already cache geocode/routes manually with bespoke keys; a shared wrapper needs per-caller review + a generic response-cache table to avoid key-mismatch dup. route_cache is structured (coords+mode), not a generic blob. | ⬜ **OPEN — deferred (needs careful per-caller work; transit-buffer correctness at stake)** |
