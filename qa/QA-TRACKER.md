...
**🟡 reorder persistence — fix deployed, LIVE VERIFY INCONCLUSIVE (C-PERSIST reorder, commit `b7ab6807e`):** added `persistDaysImmediately(newDays)` to `handleActivityReorder` so a reorder persists immediately instead of relying on the 3s autosave debounce (code-correct, mirrors the AI-note path). BUT live re-test on Madrid was inconclusive: the activity `…/⋯/Move down` menu didn't open reliably via browser automation, and after the attempts the JSON day-1 order was still unchanged (Breakfast, Check-in; 18 acts, no dup) — so I can't confirm whether Move-down even fired or whether the persist works. **⚠️ Separate concern surfaced:** the day-cost badge climbed 170→310→590 across reorder attempts (possible cost-inflation-on-reorder, likely PRE-EXISTING since it appeared before this fix too). **Next:** a clean re-test of reorder (confirm the UI order changes, then reload) + investigate the cost recompute in `handleActivityReorder`. Not claiming green.
