...
**✅ FIXED — reorder doesn't persist (C-PERSIST reorder, commit `b7ab6807e`):** Move up/down (`handleActivityReorder`) updated client state + set `hasChanges` but relied on the 3s autosave debounce, so a quick navigation/refresh lost the new order (DB kept original `sort_order`/times). Now calls `persistDaysImmediately(newDays)` right after reordering (mirrors the proven AI-note path; `hasChanges→false` so no double-save). Deployed via Vercel; ⏳ live re-verify (reorder → reload → order sticks).
