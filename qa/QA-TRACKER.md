...
| Admin — traffic / performance / forecast / projections panels | ⬜ | 🔴 | **BLOCKED 2026-06-08 — `/admin/dashboard` redirects to `/profile`.** Root cause (DB-verified): `user_roles` has ONLY `(Ashton ed60b535, role='user')` — **no `admin` role row**, so `useIsAdmin`→false → `AdminRoute` redirects ALL `/admin/*`. Owner had admin on old-Lovable; the role didn't migrate. **FIX (owner runs — privilege grant):** `INSERT INTO user_roles (user_id, role) VALUES ('ed60b535-02de-4ff5-8ccb-584a9be97a84','admin');` Then Overview/Forecast/Projections + aggregate-count re-verify can proceed. | grant admin role | 🔴 **blocked on admin-role row** |
