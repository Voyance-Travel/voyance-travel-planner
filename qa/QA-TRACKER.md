...
| C-TOOL-8 | **HIGH** | itinerary tools | **Swap/Find-Alternative doesn't persist the replacement.** Live (Florence ccfc4491): swapped "Vegan at L'OV"→"La tenda rossa", UI said "Activity swapped!", but after reload BOTH are gone (old removed, new never saved; day cost $185→$140). Core edit tool silently loses the new venue. (Free-swap accounting + 0-charge-on-1st worked.) ✅ found live 2026-06-07. | trace swap apply→persist (does it call save-itinerary with the new activity? or table/JSON divergence?) — likely a sibling of the C-PERSIST family | ⬜ |
