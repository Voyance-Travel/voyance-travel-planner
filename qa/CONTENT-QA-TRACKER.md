...
## Round 15 — travel-leg duration backstop (open item: C4)
The routing pass (geographic-coherence) computes leg times but leaves a leg blank when it can't geocode the destination (e.g. a transfer to an un-geocoded "Your Hotel" — the Tokyo build had "1/2 travel legs have no duration"). Added a backstop in the gate's repair (runs on BOTH fresh + regen paths): any travel/transport card with no duration gets a mode-based estimate (walk 10m, metro/train/bus/ferry 20m, taxi/drive 15m, airport 45m). Verified on Tokyo: "Travel to Your Hotel via Metro" → "20 min", airport transfer keeps its computed "45m", auditor TRAVEL_TIME flag clears.

## Round 16 — close the meal-time gap (open item #3)
The relabel windows had a dead gap 16:00–17:00 (between lunch-end and dinner-start), so a meal at e.g. 16:30 was left alone (and could get flagged). Closed the gap by making the windows contiguous in all three places (shared de-dup windowType, v2 inline windowType, gate WIN check): breakfast 05:00–11:30, lunch 11:00–16:30, dinner 16:30–23:30. A gap meal now relabels to the nearest meal. Verified: "Lunch at 16:45" → Dinner, "Dinner at 16:10" → Lunch, gate raises no bad-time flag after relabel. Also confirmed fresh Tokyo build live: score 100, 0 dup venues, 0/3 legs missing duration, Days 1-3 one-of-each meals (items #1 leg-fill + #2 auto-lock both validated on the fresh path).
