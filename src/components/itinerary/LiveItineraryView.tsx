...
function TimelineActivityCard({
  ...
  if (isInterCity) {
    const transportName = (activity.category || activity.type || '').includes('flight') ? 'Flight'
      : (activity.category || activity.type || '').includes('train') ? 'Train'
      : (activity.category || activity.type || '').includes('bus') ? 'Bus'
      : (activity.category || activity.type || '').includes('ferry') ? 'Ferry'
      : 'Car';
      ...
      <div className="flex gap-3">
        ...
        <div className={cn('flex-1 min-w-0', isLast ? 'pb-0' : 'pb-3')}>
          <InterCityTransportCard
            ...
            travelMeta={{
              transportName,
              depTime: activity.startTime,
              arrTime: activity.endTime,
            }}
          />
        </div>
      </div>
