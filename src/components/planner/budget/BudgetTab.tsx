        ...
        <CardContent>
          <div className="space-y-4">
            ...
            {snapshot.tripTotalCents > 0 && (() => {
              ...
              if (includeFlight) includedParts.push('Flight');
              includedParts.push('dining', 'activities', 'transit');

              const excludedParts: string[] = [];
              if (!includeHotel) excludedParts.push('hotel');
              if (!includeFlight && hasFlight) excludedParts.push('flight');

              const caption = excludedParts.length > 0
                ? `${includedParts.join(' + ')} · ${excludedParts.join(' & ')} excluded`
                : includedParts.join(' + ');

              const breakdownParts: string[] = [];
              fixedRows.forEach((r) => {
                if ((r.usedCents || 0) > 0) {
                  breakdownParts.push(`${categoryLabels[r.category]} ${formatCurrency(r.usedCents)}`);
                }
              });
              if (discretionaryCents > 0 && fixedRows.length > 0) {
                breakdownParts.push(`Discretionary ${formatCurrency(discretionaryCents)}`);
              }

              return (
                <div className="flex items-start justify-between gap-3 pt-4 mt-2 border-t-2 border-border">
                  <div className="flex items-center gap-2 min-w-0">
                    <TrendingUp className="h-4 w-4 text-muted-foreground shrink-0" />
                    <div className="min-w-0">
                      <div className="text-sm font-semibold">Trip total</div>
                            ...
                            <Tooltip delayDuration={200}>
                              <TooltipTrigger asChild>
                                <button
                                  ...
                                  aria-label="Why is this excluded?"
                                >
                              ...
                              <TooltipContent side="top" className="max-w-xs">
                                <span className="text-xs">
                                  Toggle "Include {excludedParts.join(' / ')} in Budget" in Budget Settings below to fold {excludedParts.length > 1 ? 'them' : 'it'} into this total.
                                </span>
                              </TooltipContent>
                            </Tooltip>
                          </TooltipProvider>
