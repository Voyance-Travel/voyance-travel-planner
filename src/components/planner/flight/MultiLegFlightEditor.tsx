    ...
    setSlots((prev) => {
      ...
      for (const group of connectionLegsToInsert) {
        ...
        for (const leg of group.legs) {
          const alreadyExists = next.some(s => 
            s.flight.departureAirport === leg.departureAirport &&
            s.flight.arrivalAirport === leg.arrivalAirport &&
            s.flight.flightNumber === leg.flightNumber
          );
          if (!alreadyExists) {
            newConnectionSlots.push({
              id: nextSlotId(),
              transitionIndex: -1,
              label: `Connection: ${leg.departureAirport || '?'} → ${leg.arrivalAirport || '?'}`,
              legType: 'custom',
              transportType: 'flight',
              flight: { ...leg },
              isAutoGenerated: false,
