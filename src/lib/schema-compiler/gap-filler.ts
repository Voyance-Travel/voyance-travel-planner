// src/lib/schema-compiler/gap-filler.ts
// Post-generation gap filler: detects schedule gaps > 90 min and injects
// logical filler activities (transport, hotel freshen-up, dinner).
//
// ISOLATION: Pure utility. No external dependencies beyond types.

export interface GapFillerConfig {
  minGapMinutes: number;        // Minimum gap to trigger filling (default: 90)
  hotelName?: string;           // User's hotel name for freshen-up activity
  hotelLocation?: string;       // Hotel address
  budgetTier: 'budget' | 'mid' | 'luxury';
  transportMinutes: number;     // Default transport time (default: 30)
}

export interface DetectedGap {
  afterActivity: string;
  beforeActivity: string;
  gapStartTime: string;         // HH:MM (24h)
  gapEndTime: string;           // HH:MM (24h)
  gapMinutes: number;
  dayNumber: number;
}

/**
 * Detects gaps ≥ minGapMinutes in generated activities and returns
 * filler activities to inject.
 */
...
function generateFillerActivities(gap: DetectedGap, config: GapFillerConfig): any[] {
  ...
  if (gap.gapMinutes >= 180) {
    // Full: transport → hotel → dinner → transport
    let cursor = gapStart;
