/**
 * Frontend port of `supabase/functions/_shared/chronology-validator.ts`.
 * Single canonical rule set for per-day timing coherence, used at parser
 * read-time and as a hard reject gate in the TripDetail sparse-rebuild
 * comparator.
 *
 * Keep behavior byte-equivalent with the BE module — if you change one,
 * change the other. The two cannot share a file because edge functions
 * run on Deno and frontend on Vite.
 *
...
export interface ChronologyResult<TDay = any> {
  ...
  remainingIssues: ChronologyIssue[];
  healed: boolean;
