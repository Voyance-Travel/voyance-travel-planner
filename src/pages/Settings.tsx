...
export default function Settings() {
  ...
  // Linked accounts — read the user's ACTUAL OAuth identities (Google / Apple), not a hardcode.
  // (Previously hardcoded to "Google", so Apple/email users saw the wrong provider label.)
  const [linkedAccounts, setLinkedAccounts] = useState<{ provider: string; email: string }[]>([]);
  useEffect(() => {
    let active = true;
    supabase.auth.getUser().then(({ data }) => {
      ...
      setLinkedAccounts(
        identities
          ...
          .map((i) => ({
            provider: i.provider,
            email: ((i.identity_data as Record<string, unknown> | undefined)?.email as string)
              || data.user?.email || '',
          }))
      );
    });
              ...
              <Card>
                ...
                <CardContent className="space-y-3">
                  {linkedAccounts.length === 0 ? (
                    <div className="text-sm text-muted-foreground p-3">No social sign-in is linked to this account.</div>
                  ) : linkedAccounts.map((acct) => (
                    <div key={acct.provider} className="flex items-center justify-between p-3 rounded-lg border">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="font-medium text-sm">{acct.provider === 'apple' ? 'Apple' : 'Google'}</div>
