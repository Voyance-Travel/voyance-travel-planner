...
export default function Settings() {
  ...
  // Linked accounts — show the user's ACTUAL OAuth identities (Google / Apple) and let them
  // CONNECT additional providers or UNLINK extras. Linking consolidates accounts — e.g. someone
  // who made a separate Apple "Hide My Email" account can link it to their Google account so
  // everything lives under one login. (Requires "Manual Linking" enabled in Supabase Auth.)
  const SUPPORTED_PROVIDERS: Array<'google' | 'apple'> = ['google', 'apple'];
  ...
  const providerLabel = (p: string) => (p === 'apple' ? 'Apple' : p === 'google' ? 'Google' : p);

  const handleLinkProvider = async (provider: 'google' | 'apple') => {
    setLinkBusy(provider);
    // Redirects to the provider's OAuth, then back to /settings with the identity linked.
    const { error } = await supabase.auth.linkIdentity({
      provider,
      options: { redirectTo: `${window.location.origin}/settings` },
    });
    if (error) {
      toast.error(`Couldn't connect ${providerLabel(provider)}: ${error.message}`);
      setLinkBusy(null);
    }
  };
  ...
  const handleUnlinkProvider = async (identity: UserIdentity) => {
    ...
    if (error) {
      ...
      toast.success(`${providerLabel(identity.provider)} disconnected`);
              ...
              <Card>
                ...
                <CardContent className="space-y-3">
                  {SUPPORTED_PROVIDERS.map((provider) => {
                    ...
                    const email = (identity?.identity_data as Record<string, unknown> | undefined)?.email as string | undefined;
                    const busy = linkBusy === provider;
                    const isOnlyMethod = identities.length < 2;
                    return (
                      <div key={provider} className="flex items-center justify-between p-3 rounded-lg border">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="font-medium text-sm">{providerLabel(provider)}</div>
                            <div className="text-sm text-muted-foreground">
                              {linked ? (email || 'Connected') : 'Not connected'}
                            </div>
                          </div>
                        </div>
                        {linked ? (
                          <Button
                            ...
                            title={isOnlyMethod ? "Can't disconnect your only sign-in method" : undefined}
                            onClick={() => identity && handleUnlinkProvider(identity)}
                          >
                            {busy ? '…' : isOnlyMethod ? 'Connected' : 'Disconnect'}
                          </Button>
                        ) : (
                          <Button
                            variant="default"
