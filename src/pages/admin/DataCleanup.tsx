...
export default function DataCleanup() {
  ...
  const runCleanup = async (
    ...
    if (target === 'destinations') {
      ...
      // Activities cleanup - call the edge function with countOnly to get accurate dirty count
      // The edge function uses complex logic (templated phrases, desc < 30 chars, null, bad coords)
      // that can't be replicated in a simple Supabase filter.
      const [countResponse, { count: allCount }] = await Promise.all([
        supabase.functions.invoke('cleanup-activities', {
          body: { countOnly: true, dryRun: true }
        }),
        supabase.from('activities').select('*', { count: 'exact', head: true })
      ]);

      const dirtyCount = countResponse.data?.totalCount ?? 0;
      ...
      if (typeof overallCount === 'number') {
        setDatasetCounts({
          total: overallCount,
          dirty: dirtyCount,
          clean: Math.max(0, overallCount - dirtyCount),
        });
      }
    }
    ...
    const batchSize = target === 'activities' ? 50 : target === 'attractions' ? 15 : 10;
