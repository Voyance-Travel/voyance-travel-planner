-- Update the existing handle_new_user trigger to also initialize travel_dna_profiles
-- This prevents "missing row" errors when updating these tables
...
BEGIN
  ...
  -- Create default preferences (ALREADY EXISTS - keeping for safety)
  INSERT INTO public.user_preferences (user_id)
  VALUES (NEW.id)
  ON CONFLICT (user_id) DO NOTHING;

  -- NEW: Create travel_dna_profiles row to prevent "0 rows updated" errors
  ...
  -- Log the signup bonus in the credit ledger (using correct column names)
  INSERT INTO public.credit_ledger (user_id, credits_delta, is_free_credit, action_type, transaction_type, notes)
  VALUES (NEW.id, 500, true, 'signup_bonus', 'credit', 'Welcome bonus - 500 free credits');
