...
BEGIN
  ...
  -- Check for existing collaborator record (supports rejoin)
  SELECT * INTO v_existing_collab
  FROM public.trip_collaborators
  WHERE trip_id = v_invite.trip_id AND user_id = v_user_id;

  -- If already an active member, return success with tripId so they navigate there
  ...
  -- Capacity check (only count active members, not the user themselves if they have a stale record)
  ...
  -- Add or restore as collaborator (ON CONFLICT handles rejoin)
