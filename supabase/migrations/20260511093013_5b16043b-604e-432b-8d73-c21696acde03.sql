...
COMMENT ON VIEW public.public_trip_collaborators IS
  'Peer-safe view of trip_collaborators (membership graph for current user only). Hides admin metadata invited_by + include_preferences. Base table is owner/self only. SECURITY DEFINER by design (linter 0010 accepted).';