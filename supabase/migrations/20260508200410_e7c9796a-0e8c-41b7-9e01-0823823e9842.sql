...
-- One-shot backfill: re-save trips that currently contain the leaked label,
-- so the trigger scrubs them in place.
UPDATE public.trips
SET itinerary_data = itinerary_data
WHERE itinerary_data::text ~* '(reservation[_[:space:]]?urgency|booking[_[:space:]]?(urgency|window)|lead[_[:space:]]?time)[[:space:]]*:';
