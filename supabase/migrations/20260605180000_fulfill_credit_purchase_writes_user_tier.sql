-- C-DATA-1: a successful purchase should record the buyer's tier in user_tiers.
-- The Stripe webhook calls upsertUserTier() separately, but the IAP path
-- (validate-iap-receipt) only calls fulfill_credit_purchase() — which never wrote
-- user_tiers. Result: IAP purchasers (and any purchase that bypasses the webhook
-- post-call) have no tier row, so club benefits/badges aren't tier-tracked.
--
-- Fix: have the RPC itself upsert user_tiers (covers BOTH Stripe and IAP from one
-- place). ON CONFLICT DO NOTHING is intentional — it guarantees a row exists
-- without ever DOWNGRADING a higher tier (e.g. an adventurer later buying a flex
-- top-up). The Stripe webhook's rank-aware upsertUserTier() still handles upgrades.
--
-- This is a faithful re-creation of the existing function (migration 20260302042626)
-- with ONLY the user_tiers upsert added before the final RETURN.
