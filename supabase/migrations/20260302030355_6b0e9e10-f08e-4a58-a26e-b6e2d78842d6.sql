...
-- 1) Update resolve_or_rotate_invite to use GREATEST(7, travelers * 2)
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'spotsRemaining', GREATEST(0, GREATEST(COALESCE(v_trip.travelers, 1) - 1, 0) - v_current_members),
    'rotated', p_force_rotate
  );
END;
$function$;

-- 2) Update accept_trip_invite with explicit reason codes, no accepted_at/accepted_by blocking
...
BEGIN
  ...
  -- Capacity check (exclude the rejoining user from count)
  SELECT COUNT(*) INTO v_current_members
  FROM public.trip_collaborators
  ...
  -- Metadata update only - not used for validity checks
...
-- 3) Update get_trip_invite_info to use consistent reason codes
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'spotsRemaining', GREATEST(v_max_members - v_current_members, 0)
  );
END;
$function$;

-- 4) Backfill existing active invites with low max_uses
UPDATE public.trip_invites
SET max_uses = GREATEST(7, max_uses)
