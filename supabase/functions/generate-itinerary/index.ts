...
- Use "${mcHotelName}" for the checkout activity. Do NOT invent a different hotel.${isNonFlight ? departureInstructions : ''}`;
        ...
        if (isLastDay && generatedDay.activities?.length > 1) {
          try {
            ...
            const roles = generatedDay.activities.map((a: any) => ({ act: a, role: classify(a) }));

            const breakfastItems = roles.filter((r: any) => r.role === 'breakfast');
            const checkoutItems = roles.filter((r: any) => r.role === 'checkout');
            const airportTransportItems = roles.filter((r: any) => r.role === 'airport-transport');
            const airportSecurityItems = roles.filter((r: any) => r.role === 'airport-security');
            const flightItems = roles.filter((r: any) => r.role === 'flight');
            ...
            if (breakfastItems.length > 0 && checkoutItems.length > 0) {
              ...
              if (bIdx > cIdx) {
                // Move breakfast before checkout
            ...
            if (airportTransportItems.length > 1) {
              // Keep the last one (closest to flight), remove others
              ...
              if (secIdx !== -1) {
                ...
                for (const mis of misplaced) {
                  ...
                  if (misIdx !== -1) {
                    ...
                    // Insert before airport transport or checkout
                    const atIdx = generatedDay.activities.findIndex((a: any) => classify(a) === 'airport-transport');
            ...
            if (depFlight24 && checkoutItems.length > 0) {
              ...
              if (depMins !== null) {
                const airportBuffer = 150; // Assume international (2.5h)
                const arriveAirportBy = depMins - airportBuffer;

                // Find transport duration from existing card or default 45min
                const transportCard = generatedDay.activities.find((a: any) => classify(a) === 'airport-transport');
                const transportDuration = transportCard?.durationMinutes || 45;

                const latestCheckoutMins = arriveAirportBy - transportDuration - 30;

                // Check activities between checkout and airport transport
                const cIdx = generatedDay.activities.indexOf(checkoutItems[0].act);
                ...
                if (cIdx !== -1 && tIdx !== -1 && tIdx > cIdx + 1) {
                  ...
                  // Remove activities that don't fit in the window
                  for (let j = between.length - 1; j >= 0; j--) {
                    ...
                    if (currentTime + actDur > arriveAirportBy - transportDuration) {
                      ...
                      currentTime += actDur + 15; // 15min buffer between activities
                    }
                  }
                }

                // Re-anchor checkout time if too late
                ...
                if (checkoutMins > latestCheckoutMins) {
                  ...
                  // Also adjust breakfast if present
