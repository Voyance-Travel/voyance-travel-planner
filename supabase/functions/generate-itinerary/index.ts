...
- Use "${mcHotelName}" for the checkout activity. Do NOT invent a different hotel.${isNonFlight ? departureInstructions : ''}`;
        ...
        try {
          ...
          if (bookendHotelName && generatedDay.activities?.length > 0) {
            const bActivities = generatedDay.activities;
            const bIsTransport = (a: any) => (a.category || '').toLowerCase() === 'transport';
            const bIsAccommodation = (a: any) => (a.category || '').toLowerCase() === 'accommodation';
            ...
            const bOffsetTime = (timeStr: string, minutes: number): string => {
              if (!timeStr) return '';
            ...
            const bMakeAccomCard = (label: string, startTime: string, duration: number) => ({
              ...
              startTime,
              endTime: bOffsetTime(startTime, duration),
            ...
            const bMakeTransportCard = (fromName: string, toName: string, startTime: string) => ({
              ...
              startTime,
              endTime: bOffsetTime(startTime, 15),
              durationMinutes: 15,
            ...
            for (let i = 0; i < bActivities.length - 1; i++) {
              const act = bActivities[i];
              if (bIsTransport(act) && bIsHotelRelated(act) && !bIsAccommodation(bActivities[i + 1])) {
                const accomCard = bMakeAccomCard('Freshen up at', act.endTime || bOffsetTime(act.startTime || '14:00', 15), 30);
                bActivities.splice(i + 1, 0, accomCard);
            ...
            const bVisibleActivities = bActivities.filter((a: any) => !bIsTransport(a));
            const bLastVisible = bVisibleActivities[bVisibleActivities.length - 1];
            if (bLastVisible && !bIsAccommodation(bLastVisible)) {
              const lastEndTime = bLastVisible.endTime || '22:00';
              const transportCard = bMakeTransportCard(
                bLastVisible.location?.name || bLastVisible.title || 'venue',
                bookendHotelName,
                lastEndTime
              );
              const returnCard = bMakeAccomCard('Return to', bOffsetTime(lastEndTime, 20), 15);
              bActivities.push(transportCard);
              bActivities.push(returnCard);
            ...
            const rebuilt: any[] = [];
            for (let i = 0; i < bActivities.length; i++) {
              rebuilt.push(bActivities[i]);
              
              if (i < bActivities.length - 1) {
                const curr = bActivities[i];
                const next = bActivities[i + 1];
                ...
                if (currLoc && nextLoc && currLoc !== nextLoc) {
                  const hasTransitBetween = (i + 2 <= bActivities.length - 1) && bIsTransport(bActivities[i + 1]);
                  if (!hasTransitBetween) {
                    const gapTransport = bMakeTransportCard(
