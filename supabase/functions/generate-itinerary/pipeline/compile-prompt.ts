...
${dayMealPolicy.requiredMeals.includes('breakfast') ? (breakfastHotelName ? `1. BREAKFAST (category: "dining") — At the hotel's own restaurant (preferred) or a real café nearby. NEVER at a DIFFERENT hotel's restaurant. Use the hotel name: ${breakfastHotelName}. ~price, walking distance${facts.resolvedIsHotelChange && facts.resolvedPreviousHotelName ? `\n   ⚠️ HOTEL CHANGE DAY: You are still at ${facts.resolvedPreviousHotelName} in the morning. Breakfast MUST be at ${facts.resolvedPreviousHotelName} or nearby — NOT at ${flightContext.hotelName}, which you haven't checked into yet. The correct sequence is: Breakfast at ${facts.resolvedPreviousHotelName} → Checkout → Travel → Check-in at ${flightContext.hotelName}.` : ''}` : '1. BREAKFAST (category: "dining") — At a well-reviewed local café or bakery. Do NOT reference any hotel. ~price, walking distance') : ''}
