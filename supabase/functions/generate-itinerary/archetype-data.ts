...
export function getFullArchetypeContext(
  ...
  destination?: string,
  budgetTier?: string,
  traitScores?: { pace: number; budget: number; adventure?: number; authenticity?: number; planning?: number }
): ArchetypeContext {
  // Default trait scores if not provided
  const traits = { pace: 0, budget: 0, adventure: 0, authenticity: 0, planning: 0, ...(traitScores || {}) };
