...
/** Known upscale restaurants — floor €60/pp. Includes acclaimed bistros and
 *  destination restaurants that aren't Michelin-starred but routinely run €60–90/pp,
 *  so the dining-cap doesn't quietly clip splurge intent. */
export const KNOWN_UPSCALE = /\b(il\s*gallo|ceia|enoteca|sommelier|mini\s*bar|sacramento|solar\s*dos\s*presuntos|the\s*yeatman|yeatman|sacr[ée]\s*fleur|chez\s*l['’]?ami\s*jean|le\s*comptoir\s*du\s*relais|bistrot\s*paul\s*bert|le\s*baratin|aux\s*lyonnais|allard|benoit|josephine\s*chez\s*dumonet|jos[ée]phine|le\s*train\s*bleu|brasserie\s*lipp|la\s*coupole|bofinger|lasserre|le\s*duc|le\s*violon\s*d['’]?ingres|ze\s*kitchen\s*galerie|akrame|verjus|clamato|le\s*servan|le\s*beaulieu)\b/i;
...
// Require both a leading separator AND a trailing key:value form so common
// English words (city, country, name, location, cost, type, ...) inside prose
// like "see the city from the water" are NEVER stripped. Only true JSON-leak
// fragments such as ",duration:4,city:Paris;|" match.
...
// =============================================================================
// HIGH-COST BOOKING GUIDANCE
// =============================================================================
// Premium experiences (>= $150) should always carry booking_required + a usable
// description so the UI can surface a real CTA. Prevents items like a $293 spa
// treatment from rendering as a dead-end with no description or booking link.

const HIGH_COST_USD_THRESHOLD = 150;
const HIGH_COST_GUIDANCE_CATEGORIES = new Set([
  'WELLNESS', 'SPA', 'EXPERIENCE', 'TOUR', 'CLASS', 'WORKSHOP',
  'SHOW', 'PERFORMANCE', 'ACTIVITY', 'CULTURAL',
]);

function getActivityPriceUsd(activity: Record<string, any>): number {
  const cents = typeof activity.quote_price_cents === 'number' ? activity.quote_price_cents : null;
  if (cents && cents > 0) return cents / 100;
  if (activity.cost && typeof activity.cost === 'object' && typeof activity.cost.amount === 'number') {
  ...
  const cost = typeof activity.cost === 'number' ? activity.cost
    ...
    : typeof activity.estimatedCost === 'number' ? activity.estimatedCost
    : typeof activity.price_per_person === 'number' ? activity.price_per_person
    : 0;
  return cost || 0;
}

export function enforceHighCostBookingGuidance(
  activity: Record<string, any>,
  logPrefix = 'SANITIZE'
): boolean {
  // Respect universal locking — never modify user-edited / locked items
  if (activity?.is_locked || activity?.metadata?.user_edited || activity?.metadata?.user_added) {
    return false;
  }

  const cat = (activity.category || '').toUpperCase();
  if (!HIGH_COST_GUIDANCE_CATEGORIES.has(cat)) return false;

  const price = getActivityPriceUsd(activity);
  if (price < HIGH_COST_USD_THRESHOLD) return false;
  ...
  if (desc.length < 60) {
    const niceCat = cat.toLowerCase();
    activity.description = `Premium ${niceCat} experience — reservations are typically required well in advance. Use the official site or our concierge to confirm availability and pricing.`;
    mutated = true;
  }

  activity.metadata = activity.metadata || {};
  if (!activity.metadata.booking_guidance_required) {
    activity.metadata.booking_guidance_required = true;
    mutated = true;
  }
