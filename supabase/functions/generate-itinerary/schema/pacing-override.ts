...
import type { DaySlot, PatternGroupConfig } from './types.ts';
...
export function applyPacingOverride(
  ...
  // Remove excess activity slots if over the pacing max
  ...
  // Mark activity slots as required up to the minimum
  ...
  // Add activity slots if pacing demands more than DNA provided
  ...
  // For relaxed pacing, add an unscheduled block if none exists
  if (pacing === 'relaxed') {
