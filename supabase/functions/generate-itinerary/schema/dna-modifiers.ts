...
// This file is a copy of src/lib/schema-compiler/dna-modifiers.ts with import paths
...
export function applyDnaModifiers(
    ...
    if (slot.slotType === 'meal') {
      return {
        ...
        timeWindow: slot.timeWindow
          ? { ...slot.timeWindow, duration: { ...config.mealDuration } }
          : null,
      };
    }
    return slot;
  ...
  modified = modified.map(slot => {
    if (slot.slotType === 'meal' && slot.mealType === 'breakfast' && slot.timeWindow) {
      ...
      return {
        ...slot,
        timeWindow: { ...slot.timeWindow, earliest: breakfastEarliest, latest: breakfastLatest },
      };
    }
    return slot;
  });
