...
// This file is a copy of src/lib/schema-compiler/prebooked-filler.ts with import paths
...
import { parseTimeToMinutes, subtractMinutes } from './time-parser.ts';
...
function fillDiningCommitment(slots: DaySlot[], commitment: PreBookedInput): DaySlot[] {
  ...
  if (mealIdx >= 0) {
    ...
    // No matching meal slot found — insert as a new slot
...
function fillActivityCommitment(
  ...
  // Find an empty activity slot near this time
  let targetIdx = -1;
  ...
  if (targetIdx >= 0) {
    ...
    // Insert a new slot at the right position
    const insertIdx = findInsertionPoint(slots, commitment.startTime);
    const newSlot: DaySlot = {
      slotId: `prebooked_${commitment.title.toLowerCase().replace(/\s+/g, '_').substring(0, 20)}`,
      slotType: 'activity',
  ...
  // Reverse-schedule transport to the commitment
  ...
  // Find the commitment slot and check if transport is needed before it
  ...
  if (commitmentIdx > 0) {
    ...
    // Only add transport if the previous slot isn't already transport to this location
