...
export async function prepareContext(supabase: any, tripId: string, userId?: string, directTripData?: DirectTripData, requestSmartFinishMode?: boolean): Promise<GenerationContext | null> {
  ...
  // into the structured `trip_day_intents` table here so the rest of the
  // pipeline (compile-prompt, day-ledger, day-brief checker) can rely on
  // ONE source of truth — per-day, per-source rows — instead of re-parsing
  // blobs in three places. Idempotent: the upsert dedupes via unique index.
  try {
    const { intentsFromChatPlannerExtraction, intentsFromFineTuneNotes, intentsFromUserAnchors } =
      await import('../_shared/intent-normalizers.ts');
    const { upsertDayIntents } = await import('../_shared/day-intents-store.ts');
    const md = (trip.metadata || {}) as Record<string, any>;
    const seedIntents: any[] = [];

    // Chat-planner extraction (perDayActivities + mustDoActivities + userConstraints)
    ...
    // Fine-tune textarea notes (parsed for "Day N" markers)
    ...
    // Manual paste / pin / edit anchors (already locked rows)
    ...
    if (seedIntents.length > 0) {
      ...
      console.log(`[Stage 1] Seeded trip_day_intents: ${written}/${seedIntents.length} rows from metadata`);
    }
  } catch (seedErr) {
