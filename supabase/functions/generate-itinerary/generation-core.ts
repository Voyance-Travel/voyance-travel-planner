...
export async function generateSingleDayWithRetry(
  ...
  const qualityRules = [
    ...
    isLastDay && context.totalDays > 1 ? '12. **LAST DAY DEPARTURE TIMING — CRITICAL**: The day MUST end with Checkout → Transfer → Departure in that order. Hotel checkout MUST be at or before 11:00 AM (never afternoon or evening). The transfer to the airport/station MUST be timed backwards from departure: arrive at airport 3 hours before flight (2 hours for train), so transfer starts at (departure − 3h − transferDuration). NEVER schedule any leisure activity (dining, sightseeing, freshen-up, walks) AFTER the airport/station transfer card. If no flight time is provided, default checkout to 11:00 AM and do NOT invent a generic "Transfer to Airport" placeholder.' : '',
    ...
    try {
      ...
      const comprehensiveConstraints = buildAllConstraints(
        ...
        context.budgetTier,
        {
          ...
          planning: context.travelerDNA?.traits?.planning || 0
        },
        context.destination
      );
...
Generate activities for this day following ALL constraints above.`;
      ...
      if (validation.isValid || (isLastAttempt && !smartFinishBlocksReturn)) {
        ...
        enforceDayTitleCoherence(generatedDay, { city: (dayCity?.cityName as string) || context.destination || '' });
        return generatedDay;
      }
...
export async function finalSaveItinerary(
  ...
  try {
    ...
    // Delegated to shared helper so per-day-chain path produces identical rows.
    // =========================================================================
    try {
      const { writeActivityCostsFromItinerary } = await import('../_shared/write-activity-costs.ts');
      const res = await writeActivityCostsFromItinerary(supabase, tripId, enrichedData.days || [], {
        destination: context.destination || '',
        travelers: context.travelers || 1,
        budgetTier: context.budgetTier,
        actualDailyBudgetPerPerson: context.actualDailyBudgetPerPerson ?? null,
      });
      console.log(`[Stage 6] activity_costs writer: inserted=${res.inserted} reason=${res.skippedReason ?? 'ok'}`);
    } catch (costWriteErr) {
