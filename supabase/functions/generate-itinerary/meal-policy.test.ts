...
Deno.test('meal policy: tight 10:30 departure requires no meals (breakfast cannot fit before the airport run)', () => {
  // A 10:30 flight with the standard ~180min airport buffer + transfer means
  // leaving the hotel ~06:45 — earlier than a realistic breakfast window — so the
  // breakfastFits gate correctly requires no mandatory meal (avoids a false
  // "missing breakfast" on a tight morning departure).
  const policy = deriveMealPolicy({
    dayNumber: 4,
    totalDays: 4,
    isFirstDay: false,
...
Deno.test('meal policy: a roomy late-morning departure (12:00) still gets breakfast', () => {
  // By noon the breakfast window comfortably fits before the airport run.
  const policy = deriveMealPolicy({
    dayNumber: 4,
    totalDays: 4,
    isFirstDay: false,
    isLastDay: true,
    departureTime24: '12:00',
  });

  assertEquals(policy.requiredMeals, ['breakfast']);
});

Deno.test('meal policy: very early departure requires no meals', () => {
  const policy = deriveMealPolicy({
    dayNumber: 4,
    totalDays: 4,
    isFirstDay: false,
    isLastDay: true,
    departureTime24: '07:00', // wheels-up before breakfast is feasible
  });

  assertEquals(policy.requiredMeals, []);
});

Deno.test('meal policy: last day with NO departure time requires no mandatory meals', () => {
  // No return flight set → we cannot anchor the day, so we must NOT false-require
  // breakfast/lunch (that produced the "missing breakfast Day 4" false positive).
  const policy = deriveMealPolicy({
    dayNumber: 4,
    totalDays: 4,
    isFirstDay: false,
    isLastDay: true,
    // departureTime24 intentionally omitted
  });
