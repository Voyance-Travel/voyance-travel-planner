/**
 * inferArrivalMinsFromSchedule — backend mirror of
 * src/lib/itinerary/inferArrivalFromSchedule.ts.
 *
 * Returns start time (in minutes since midnight) of the first activity that
 * represents real "experience" content, skipping bookend / logistics cards.
 * Used by save-time meal-policy reconciliation when no flight clock is
 * available so a synthetic "Travel to Hotel 08:00" card never inflates the
 * inferred arrival backward.
 */
...
/**
 * Convert minutes-since-midnight to "HH:MM" (24h). Returns undefined for
 * null/invalid input so callers can pass directly into deriveMealPolicy.
 */
export function minsToClock24(mins: number | null | undefined): string | undefined {
  if (mins === null || mins === undefined || Number.isNaN(mins)) return undefined;
