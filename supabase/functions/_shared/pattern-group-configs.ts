// Pattern Group Configs — Backend Mirror.
// Source of truth: src/config/pattern-group-configs.ts (Fix 22A-E).
// Keep in sync by convention.

import type { PatternGroupConfig } from './schema-generation.ts';
...
export const BALANCED_CONFIG: PatternGroupConfig = {
  ...
  dayEndTime: '22:00',
  mealWeight: 'standard',
  mealInstruction: 'Find a well-reviewed local restaurant. Good food, not rushed, not over-the-top.',
  bufferMinutes: 30,
  unscheduledBlocks: 0,
  hotelPriority: 'first',
...
export const INDULGENT_CONFIG: PatternGroupConfig = {
  ...
  mealInstruction: 'Find an exceptional dining experience. Reservations, ambiance, culinary reputation matter. This IS the activity.',
  bufferMinutes: 30,
  unscheduledBlocks: 0,
  hotelPriority: 'first',
  breakfastRequired: true,
...
export const GENTLE_CONFIG: PatternGroupConfig = {
  ...
  dayEndTime: '21:00',
  mealWeight: 'standard',
  mealInstruction: 'Find a comfortable, unhurried spot. No loud or crowded venues. Quality over scene.',
  bufferMinutes: 60,
  unscheduledBlocks: 1,
  hotelPriority: 'first',
  breakfastRequired: true,
...
export function getPatternGroupConfig(group: string | null | undefined): PatternGroupConfig {
  if (!group) return BALANCED_CONFIG;
